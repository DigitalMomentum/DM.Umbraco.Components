angular.module('umbraco').directive('dmImageButton', function (entityResource, mediaHelper, dialogService) {
	return {
		scope: {
			value: '=',
			config: '='
		...
		restrict: 'E',
		replace: true,
		templateUrl: '/App_Plugins/DM.ImageButton/Views/Directive.html',
		link: function (scope) {
			...
			scope.selectLink = function () {
				dialogService.linkPicker({
					...
					callback: function (data) {
						scope.value.link = data;
					}
			});
			}

			scope.removeLink = function () {
				scope.value.link = null;
			};
